...
class GA_partition(object):
    ...
    def subtract_sum(self, chromosome):
        try:
            value = -(self.bag_capacity - chromosome.T @ self.data)
        except:
            value = None
            print('chro:',chromosome.shape)
    ...
    def parallel_VNS(self, population):
        local_optimums = np.array([[]] * self.length)

        nums_thread = 10
        factor = int(population.shape[1] / 10)
        threads = []
        for i in range(nums_thread):
            if i == nums_thread - 1:
                threads.append(MyThread(self.variable_neighborhood_search, (population[:, i*factor:(i+1)*factor],) ))
            else:
                temp = population[:, i*factor:]
                threads.append(MyThread(self.variable_neighborhood_search, (temp,)))
        for i in range(nums_thread):
            threads[i].start()
        for i in range(nums_thread):
            threads[i].join()
        ...
        for i in range(nums_thread):
            x = threads[i].get_result()
            local_optimums = np.c_[local_optimums, x]
        return local_optimums

    def parallel_VNS2(self, population):
        result = self.pool.map(self.variable_neighborhood_search2, population.T)
    ...
    def variable_neighborhood_search2(self, instance):
        # 遍历所有可行解
        local_optimums = np.array([[]] * self.length)
    ...
    def main(self):
        ...
        while iteration < self.iteration:
            ...
            mutation_children = self.parallel_VNS2(mutation_children)
...
def test_dataset():
    ...
    np.savetxt(os.path.join(output_folder, 'test_parallel_output.csv'), delimiter=',', fmt='%s')
